...
class ObservationSettings(HoconConvertible):
    ...
    Attributes:
        ...
        - event_timestamp_column (Optional[str]): The timestamp field of your record.
        As sliding window aggregation feature assume each record in the source data should have a timestamp column.
        - timestamp_format (Optional[str], optional): The format of the timestamp field. Defaults to "epoch".
        Possible values are:
    ...
    def __init__(self,
        ...
        if observation_path.startswith("http"):
            logger.warning("Your observation_path {} starts with http, which is not supported. "
                           "Consider using paths starting with wasb[s]/abfs[s]/s3.", observation_path)
